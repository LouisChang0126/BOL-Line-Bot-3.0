...
- ➡️ Ctrl+Y 重做
- 按鈕也可使用
- 📦 **全面涵蓋**：人員增刪移動、新增/刪除服事欄位、新增/刪除一週、服事排序、剪下/貼上、Delete 清空
...
- 📋 **智慧貼上**：
  - 從**本站複製**的資料直接貼上（無需確認）
  - 從 **Excel/Google Sheet** 貼上時開啟預覽 Modal，可即時切換分隔符（`/`、`+`、`,`、`，`、空格、無分隔）
- 🔲 **多格選取**：長按空白格拖拉選取多格，支援 Ctrl+C 複製 / Ctrl+X 剪下 / Delete 清空
- 📋 **右鍵選單**：複製、剪下、貼上操作
...
- 📅 **參考範圍 / 生成範圍**（排班模式專屬）：
  ...
  - **參考週次**：限縮 LLM 看到的班表（input filter），預設第一週至最後一個非空白週；按過「📅 顯示歷史」載入歷史資料後，下拉會多出歷史週次可選
  - **生成週次**：限縮 LLM 只能改這幾週（output scope），可選擇尚未存在的未來週日，前端會自動建立空週次再打 API
  - 起訖選項會即時連動隱藏（start 不能晚於 end）
- 🚫 **請假區域**（排班模式專屬，預設摺疊）：
  - 雙下拉設計：左側選日期（從生成週次推算）、右側選人員（從歷史人名）、右側「×」可移除該列
...
### v4.3.0 (2026-04-27)
- 🆕 **AI 助手「請假區域」功能**（排班模式）：
  - 雙下拉設計（日期 + 人員）+「×」移除 +「+ 新增請假」可動態加列
  - 預設摺疊；展開後預設只有 1 列空白
  - 容器 max-height + overflow scroll，多列也不撐版面
  - 送出驗證：每列要全填或全空，半空擋下；同日期多列自動合併
  - 後端在 system prompt 加 `Person Unavailability` 硬性規則 + `_validate_tool_input` 422 雙重把關
  - 前端 validator 加 `personUnavailability` 規則類型，違反時觸發既有重試流程
- 🆕 **參考週次支援歷史資料**：按過「📅 顯示歷史」載入後，參考週次下拉會多出歷史週次可選；送 API 時自動把歷史資料併入 `currentSchedule` 範圍
